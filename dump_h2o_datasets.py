...
parser.add_argument('--no-split', default=False ,action='store_true', help="Dump a non-split dataset.")

LOOKBACK = int(parser.parse_args().lookback)
LOOKFWD = int(parser.parse_args().lookfwd)
RESAMPLE = parser.parse_args().resample
NO_SPLIT = parser.parse_args().no_split
...
if NO_SPLIT:
    ds = DataSet(tickers=tickers, lookback=LOOKBACK, lookfwd=LOOKFWD, predicate=f"date > '1960-01-01'", resample=RESAMPLE).data
    print('\n--------------------------- Dataset ---------------------------')
    print(ds.describe())
